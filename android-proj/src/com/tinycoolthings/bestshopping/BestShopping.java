...
public class BestShopping extends Application {

	private static BestShopping instance = null;
	 ...
	 */
	public static BestShopping getInstance() {
		checkInstance();
		return instance;
	}
	...
	public void setAppContext(Context ctx) {
        BestShopping.appContext = ctx;
	}

	public Context getAppContext() {
		return BestShopping.appContext;
	}

	public void search(String text) {
		text = text.trim();
		if (!Utils.validSearch(text)) {
			AlertDialog.Builder altDialog = new AlertDialog.Builder(BestShopping
			...
			CallWebServiceTask search = new CallWebServiceTask(Constants.Actions.SEARCH,
					false);
			search.addParameter(Constants.Server.Parameter.Name.SEARCH_QUERY, text);
			search.execute();
		}
	}
	...
	public void showWaitingDialog() {
		if (this.waitingDialog == null) {
			this.waitingDialog = ProgressDialog.show(BestShopping.getInstance()
                    .getAppContext(), BestShopping.getInstance().getResources()
                    .getString(R.string.loading), BestShopping.getInstance()
	...
	public List<Product> getProductsFromIDsList(List<Integer> productsIDs) throws SQLException {
		QueryBuilder<Product, Integer> queryBuilder = databaseHelper
				.getProductRuntimeDao().queryBuilder();
	...
	public List<Category> getCategoriesFromIDsList(List<Integer> categoriesIDs,
		...
		QueryBuilder<Category, Integer> queryBuilder = databaseHelper
				.getCategoryRuntimeDao().queryBuilder();
		switch (sortType) {
			case Constants.Sort.NAME_ASCENDING:
				queryBuilder.orderBy(Product.NAME_FIELD_NAME, true);
				break;
			case Constants.Sort.NAME_DESCENDING:
