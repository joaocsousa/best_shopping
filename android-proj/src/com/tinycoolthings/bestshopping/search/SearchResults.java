...
import com.tinycoolthings.bestshopping.MainMenu;
import com.tinycoolthings.bestshopping.models.Product;
import com.tinycoolthings.bestshopping.product.ProductView;
...
public class SearchResults extends SherlockFragmentActivity {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(Constants.Actions.GET_PRODUCT)) {
				...
				try {
					...
					Product product = BestShopping.getInstance().addProduct(prodJson);
				...
				Product selectedProd = BestShopping.getInstance().getProductById(selectedProdID);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mActionBar.setTitle(BestShopping.getInstance().getLatestSearchTerm());
	...
	protected void onResume() {
		...
		BestShopping.getInstance().setAppContext(SearchResults.this);
	}
        ...
        protected Void doInBackground(Void... params) {
			try {
				...
				for (int i=0;i<prodMarcaJSON.length();i++) {
					products.add(BestShopping.getInstance().addProduct(prodMarcaJSON.getJSONObject(i)));
				}
				
				for (int i=0;i<prodNomeJSON.length();i++) {
					products.add(BestShopping.getInstance().addProduct(prodNomeJSON.getJSONObject(i)));
				}
